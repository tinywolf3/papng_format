...
function renderInfo(next:Info) {
  ...
  element('metadata-panel').hidden=!next.metadata;
  element('metadata-source').textContent=next.metadata?.text??'';
  element('metadata-note').textContent=next.metadata?.truncated?'긴 메타데이터의 앞 65,536자만 표시합니다.':'파일에 저장된 주석과 개발자 참고 정보입니다. 읽기 전용으로 표시합니다.';
...
async function load(buffer:ArrayBuffer,name:string,sample?:Sample) {
  ...
  element('overlay').replaceChildren();element('socket-panel').hidden=true;element('metadata-panel').hidden=true;element('metadata-source').textContent='';
