...
import type { Clip, Hints, MaskGroup, Sockets } from '../../../../packages/papng/src/types';
export type Command =
...
export interface Info {name:string;width:number;height:number;frames:number;bytes:number;maskCount:number;maskColors:MaskColor[];maskMaps:number;maskBindings:number;clips:Clip[];groups:MaskGroup[];sockets?:Sockets;hints:Hints;distributions:{kind:number;valid:boolean}[];controls:{frame:number;type:number}[]}
