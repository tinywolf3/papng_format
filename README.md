...
- **16비트 마스크:** 원본 RGBA를 보존하고, 별도 압축 배열의 마스크 인덱스별로 HSV 변화량을 적용합니다. ΔS·ΔV는 −1~+1의 가산 차이이며 결과는 0~1로 제한합니다. 기본값은 모두 0입니다. 픽셀별 알파를 유지하며 최대 32,768개 마스크를 지원합니다.
...
일반 APNG 디코더에서도 컨테이너를 읽을 수 있습니다. 원본 색상과 알파는 그대로 보이며, HSV 편집과 확장 재생 동작에는 PAPNG를 지원하는 구현이 필요합니다.

### 포맷 명세

- [PAPNG 1.1 한국어 명세 — 기준 문서](spec/PAPNG-1.1.ko.md)
- [PAPNG 1.1 영어 명세 — 번역본](spec/PAPNG-1.1.en.md)
...
- [TypeScript 웹 뷰어](demos/web/viewer/README.md): 로컬 파일 재생, 마스크 HSV 변화량 편집, 클립 선택, 소켓에 부속 연결과 점프 대상 우선 캐시를 시험할 수 있습니다.

Node.js 22.12 이상에서 `npm ci` 후 `npm run dev`로 실행합니다. 디코더와 플레이어는 포맷 동작을 확인하기 위한 실험용 구현입니다.

이전 명세: [1.0 한국어](spec/PAPNG-1.0.ko.md) · [1.0 English](spec/PAPNG-1.0.en.md)
...
It preserves original RGBA and adds independently compressed, shareable 16-bit mask planes, reusable random distributions, frame controls, and optional JSON5 metadata including named sockets with per-frame position and rotation. Comments and additional members can carry developer notes. Up to 32,768 logical masks accept runtime HSV offsets while keeping per-pixel alpha. S/V use additive offsets from −1 to +1 and clamp the result to 0–1. All offsets default to zero. Ordinary APNG decoders show original colors; HSV edits and extended playback require a PAPNG-aware implementation.

### Specification

- [PAPNG 1.1 — Korean normative specification](spec/PAPNG-1.1.ko.md)
- [PAPNG 1.1 — English translation](spec/PAPNG-1.1.en.md)
...
- The [TypeScript web viewer](demos/web/viewer/README.md) provides local playback, HSV-offset editing, clip selection, and a cache that favors jump targets.
